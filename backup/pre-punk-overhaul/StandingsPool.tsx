...
//
// The NBA standings prediction pool. Visual language: warm charcoal panels
// (courtside at night), cream court paint, leather basketball orange —
// deliberately not the standard glassy AI look.
//
// Structure:
...
function LiveStandingsList({ conf, rows }: { conf: Conference; rows: NbaStandingRow[] }) {
  return (
    <div className="space-y-2">
      <div className="px-1 font-mono text-[9px] uppercase tracking-[0.25em] text-cream/40">
        Live standings
      </div>
      <ol className="space-y-1 rounded-lg border border-cream/10 bg-black/25 p-1">
        {rows.map((row, index) => {
          ...
          return (
            ...
            >
              <span className="w-5 shrink-0 text-center font-mono text-[10px] text-cream/35 tabular-nums">
                {index + 1}
              </span>
              <TeamChip team={team} size="sm" />
              <span className="min-w-0 flex-1 truncate text-[13px] font-medium text-cream/70">
                {team.name}
              </span>
              <span className="shrink-0 font-mono text-[9px] tabular-nums text-cream/30">
                {row.wins}-{row.losses}
              </span>
            </li>
          );
...
/** Plaque at the top of each conference column. */
function ConferencePlaque({ conference }: { conference: Conference }) {
  return (
    <div
      className={`rounded-md border px-3 py-2 text-center font-mono text-[11px] uppercase tracking-[0.28em] ${
        conference === "West"
          ? "border-pool-orange/50 bg-pool-orange/15 text-pool-orange"
...
function ParticipantBoard({
  ...
  const renderTeam = (teamId: string, index: number, conf: Conference) => {
    ...
    const accuracyClass = !seasonStarted
      ? null
      : rankDiff === 0
        ? "border-pinGreen/70 bg-pinGreen/10"
        : rankDiff === 1
          ? "border-pinGold/70 bg-pinGold/10"
          : null;
    ...
    return (
      <li
        ...
        className={`group flex items-center gap-2 rounded-lg border px-2 py-1.5 transition-all ${
          isDragging
            ? "border-pool-orange/70 bg-pool-orange/25 opacity-40"
            : isDropTarget
              ? "border-pool-orange/80 bg-pool-orange/20 shadow-[0_0_0_3px_rgba(232,100,47,0.25)]"
              : (accuracyClass ??
                  (canDrag
                    ? "cursor-grab border-cream/15 bg-cream/[0.04] shadow-[0_2px_8px_rgba(0,0,0,0.35)] hover:border-pool-orange/50 hover:bg-pool-orange/10 active:cursor-grabbing"
                    : "border-cream/10 bg-cream/[0.03]"))
        }`}
      >
        <span className="w-5 shrink-0 text-center font-mono text-[10px] text-cream/45 tabular-nums">
          {rank}
        </span>
        <TeamChip team={team} size="sm" />
        <span className="min-w-0 flex-1 truncate text-[13px] font-medium text-cream/90">
          {team.name}
        </span>
        {showLive && (
          <span
            className="shrink-0 font-mono text-[9px] uppercase tracking-wider text-cream/40"
            title="Live standing"
          >
            #{liveRank}
          </span>
        ...
        {canDrag && (
          <span
            aria-hidden
            className="shrink-0 font-mono text-xs leading-none text-cream/25 group-hover:text-cream/50"
          >
            ⠿
          </span>
        )}
  ...
  return (
    <section
      className={`overflow-hidden rounded-xl border transition-colors ${
        isActive
          ? "border-pool-orange/60 bg-pool-ink2 shadow-[0_10px_36px_-12px_rgba(232,100,47,0.35)]"
          : "border-cream/10 bg-pool-ink/85 hover:border-cream/25"
      }`}
    >
      <header className="flex items-center justify-between gap-3 border-b border-cream/10 bg-black/20 px-4 py-3">
        <div className="flex min-w-0 items-center gap-2.5">
          <h3 className="truncate font-sign text-xl uppercase tracking-wide text-cream">{name}</h3>
          {saved ? (
            <span className="shrink-0 rounded-full border border-pinGreen/40 bg-pinGreen/10 px-2 py-0.5 font-mono text-[9px] uppercase tracking-wider text-pinGreen">
              saved
            </span>
          ) : (
            <span className="shrink-0 rounded-full border border-cream/20 bg-cream/5 px-2 py-0.5 font-mono text-[9px] uppercase tracking-wider text-cream/50">
              chalk
            </span>
          )}
        </div>
        <div className="flex shrink-0 items-center gap-2">
          {updatedAt && (
            <time className="hidden font-mono text-[9px] uppercase tracking-wider text-cream/35 sm:inline">
          ...
          <button
            ...
            className={`rounded-full border px-3 py-1.5 font-mono text-[10px] uppercase tracking-widest transition-colors ${
              isActive && isUnlocked
                ? "border-pinGreen/50 bg-pinGreen/10 text-pinGreen hover:bg-pinGreen/20"
                : "border-pool-orange/50 text-pool-orange hover:bg-pool-orange/15"
            }`}
          >
            {isActive && isUnlocked ? "editing" : isUnlocked ? "edit" : "Unlock edit"}
          </button>
        </div>
      </header>        <div className="grid gap-4 p-4 sm:grid-cols-2">
        {columns.map((conf) => {
            ...
            <div key={conf} className="space-y-2.5">
              ...
              <ol
                ...
                className={`space-y-1.5 rounded-lg p-1 transition-colors ${
                  isTarget ? "bg-pool-orange/10 outline-2 outline-dashed outline-pool-orange/50" : ""
                }`}
              >
...
function Leaderboard({
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-cream/15 bg-pool-ink">
      <div className="flex flex-wrap items-baseline justify-between gap-2 border-b border-cream/15 px-4 py-3 sm:px-5">
        <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-pool-orange">
          Leaderboard
        </span>
        <span className="font-mono text-[9px] uppercase tracking-widest text-cream/50">
        ...
        {ranked.map((row, i) => (
          <li key={row.name}>
            <button
              ...
              className={`group flex w-full items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-cream/[0.06] sm:px-5 ${
                i % 2 === 1 ? "bg-cream/[0.03]" : ""
              }`}
            >
              <span
                className={`w-6 shrink-0 text-center font-sign text-lg ${
                  i === 0 ? "text-pool-orange" : "text-cream/40"
                }`}
              >
                {i + 1}
              </span>
              ...
              <span
                className={`w-20 shrink-0 rounded-md px-2 py-1 text-center font-sign text-lg tabular-nums ${
                  i === 0 && row.points > 0
                    ? "bg-pool-orange text-pool-ink"
                    : "border border-cream/15 bg-cream/5 text-cream"
                }`}
              >
                {row.points}
                <span className={`ml-0.5 font-mono text-[9px] ${i === 0 && row.points > 0 ? "text-pool-ink/60" : "text-cream/40"}`}>
                  /{row.max}
                </span>
              </span>
              <span
                aria-hidden
                className="shrink-0 font-mono text-sm text-cream/30 transition-colors group-hover:text-pool-orange"
              >
                →
              </span>
            </button>
    ...
    >
      <div className="my-8 w-full max-w-6xl animate-modal-pop">
        <div className="flex items-stretch justify-center gap-4">
          {/* West live standings rail */}
          <aside className="hidden w-56 shrink-0 xl:block">
            <div className="rounded-xl border border-cream/10 bg-pool-ink/85 p-2.5">
          ...
          {editable && (
            ...
            {saveError && (
              <p role="alert" className="mr-auto font-mono text-xs text-pool-orange">
                {saveError}
              </p>
            )}
            <button
              type="button"
              onClick={onDone}
              className="rounded-full border border-cream/25 px-4 py-2 font-mono text-[10px] uppercase tracking-widest text-cream/60 hover:text-cream"
            >
              Done
            </button>
            <button
              ...
              onClick={onSave}
              disabled={saving}
              className="rounded-full bg-pool-orange px-5 py-2 font-mono text-[10px] uppercase tracking-widest text-black transition-colors hover:bg-cream disabled:opacity-50"
            >
  ...
  return createPortal(
    ...
    >
      <form
        ...
        className="w-full max-w-sm rounded-2xl border-2 border-pool-orange/40 bg-pool-ink2 p-6 shadow-[0_24px_70px_-18px_rgba(0,0,0,0.9)] animate-modal-pop"
      >
        <div className="flex items-center justify-between">
          <BasketballIcon className="size-9 animate-ball-bounce" />
          <button
            type="button"
            onClick={onClose}
            aria-label="Close"
            className="flex size-8 items-center justify-center rounded-full border border-cream/25 font-mono text-base text-cream/70 hover:border-cream/60 hover:text-cream"
          >
            ×
          </button>
        </div>
        <h2 id="pool-unlock-title" className="mt-4 font-sign text-2xl uppercase tracking-wide text-cream">
          Unlock your board
        </h2>
        ...
        <input
          ...
          className="mt-4 min-h-11 w-full rounded-lg border border-cream/20 bg-black/30 px-3 font-mono text-sm text-cream placeholder:text-cream/35"
        />
        {error && (
          <p role="alert" className="mt-2 font-mono text-xs text-pool-orange">
            {error}
          </p>
        )}
        <button
          ...
          className="mt-4 min-h-11 w-full rounded-full bg-pool-orange py-3 font-sign text-base uppercase tracking-[0.2em] text-black transition-colors hover:bg-cream disabled:cursor-not-allowed disabled:opacity-40"
        >
...
export default function StandingsPool() {
  ...
  if (loading) {
    return (
      <div className="flex min-h-[50vh] items-center justify-center rounded-2xl border border-cream/15 bg-pool-ink">
  ...
  if (loadError || !data) {
    return (
      <div className="rounded-2xl border border-cream/15 bg-pool-ink p-8 text-center">
        <p className="font-sign text-2xl uppercase text-cream">Airball.</p>          <p className="mt-2 font-mono text-xs text-cream/60">{loadError ?? "No data"}</p>
        <button
          type="button"
          onClick={() => {
            setLoadError(null);
          ...
          className="mt-5 rounded-full bg-pool-orange px-6 py-2.5 font-mono text-xs uppercase tracking-widest text-black hover:bg-cream"
        >
          Run it back
        </button>
      </div>
  ...
  return (
    <div className="space-y-6">
      {/* Title card — plain black panel for now. */}
      <header className="rounded-2xl border border-cream/15 bg-pool-ink px-6 py-10 text-center sm:px-10">
        <h1 className="font-sign text-4xl font-bold uppercase leading-none tracking-tight text-cream sm:text-6xl">
          Gradey Dick Fan Club Predictions
        </h1>
        <p className="mt-3 font-mono text-[11px] uppercase tracking-[0.25em] text-cream/70">
      ...
      {data.dbError && (
        <div role="alert" className="rounded-xl border border-pinRed/50 bg-pinRed/10 px-4 py-3 text-sm text-cream/85">
          <p>{data.dbError}</p>
          <p className="mt-1.5 font-mono text-[10px] uppercase tracking-wider text-cream/50">
      ...
      {locked && (
        <div className="rounded-xl border border-cream/20 bg-black/20 px-4 py-3 text-center font-mono text-xs uppercase tracking-widest text-cream/60">
